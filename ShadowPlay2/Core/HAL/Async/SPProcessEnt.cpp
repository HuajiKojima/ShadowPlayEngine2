...
namespace ShadowPlay
{
    ...
    )
    {
        ...
        try
        {
            spThread->thread = new std::thread([spThread]()
                {
                    //std::unique_lock<std::mutex> lock(spThread->threadGlobalMutex);
                    //spThread->threadGlobalCV.wait(lock, [spThread]{ return !(spThread->bIsRunning.load()); });
                    //lock.unlock();
    ...
    {
        bIsRunning.store(bShouldPause);
        threadGlobalCV.notify_all();
    }
